...
//  pinMode(AnalogInPin,INPUT);
  pinMode(AnalogVpin,INPUT);
  pinMode(AnalogCpin,INPUT);
...
void loop(){
  ...
  int VsValue=analogRead(AnalogVpin);
  //CURRENT SENSOR
  //Read current sensor values 
  
  int CsValue=analogRead(AnalogCpin);  
  ...
  {
    digitalHigh(M1BRK);
    analogWrite(TAILPWM,0);
  }
 

//ENCODER MOTOR LEFT
  if (inpen > 0 && inpen < 1530)   //sets limit to input angle
  {
    analogWrite(M1PWM,100);
    analogWrite(M2PWM,100); 
    while(1)                      //rotates encoder motor left
   {
    long EnL_Pos=EnL.read();
    ...
    if(abs(EnL_Pos)>=1600)                  //prevents excessive left encoder motor rotation
      {
        analogWrite(M2PWM,0);
        digitalHigh(M2BRK);
   ...
   while(1)                                     //rotates encoder motor right
   {
    ...
    if (abs(EnR_Pos)>=1600)                     //prevents excessive right motor rotation
      {
        analogWrite(M1PWM,0);
        digitalHigh(M1BRK);
...
//  else                                      //when serial not available
//  {
//    inptail=0;
//    inpen=0;
//    digitalHigh(M1BRK);
...
//    }
  ...
  count = count + 1;                              //average values after 50 readings
  if (count == 50)
  {
    count = 0;
