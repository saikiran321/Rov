...
//ENCODER MOTOR LEFT
  ...
  { 
   ...
   // Serial.println(inpen);
    digitalLow(M1BRK);
    digitalLow(M2BRK);
    if (direction_reversed_M1 == 0)
    {
      digitalHigh(M1Dir);   //change according to direction
    }
    if (direction_reversed_M2 == 0)
    {
      digitalLow(M2Dir);   //change according to direction
    }
    ...
    if ((abs(EnL_Pos) - abs(inpen))==0 && (abs(EnR_Pos) - abs(inpen))==0)
    {
      digitalHigh(M1BRK);
      digitalHigh(M2BRK);
      break;
    ...
    {
      differenceL = (abs(EnL_Pos) - abs(inpen));
      if ((abs(differenceL) - abs(differenceL_P)) > 0)
      {
        digitalHigh(M2Dir);
        direction_reversed_M2 = 1;
      }
      differenceR = (abs(EnR_Pos) - abs(inpen));
      if ((abs(differenceR) - abs(differenceR_P)) > 0)
      {
        direction_reversed_M1 = 1;
        digitalLow(M1Dir);
      }
      differenceR_P = differenceR ;
      differenceL_P = differenceL ;
      }
   }
    ...
    differenceR_P = 0;
    differenceL_P = 0;
    direction_reversed_M1 = 0;
    direction_reversed_M2 =0;
